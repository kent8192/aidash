...
The policy service implements policy management, decisions, revocable subject credentials, scoped workspace and interaction APIs, tenant catalog approvals and local execution for FR-AUTH-001. Management endpoints require the existing operator bearer token or an explicitly granted OIDC browser operator session. Remote federation, finer resource scopes and remaining management workflows remain under development; this is not completion of FR-AUTH-001.

## Dashboard identity

`GET /auth/config` reports whether OIDC is configured. `GET /auth/login` starts Authorization Code with PKCE, and `/auth/callback` consumes the browser-bound transaction and creates an opaque Aidash session. `GET /auth/session` lists only the signed-in identity's enabled mappings and operator availability. An unmapped identity may read and submit its own request through `GET` and `POST /auth/registration` but cannot access `/api` data. Browser mutations send the session's `aidash-csrf` value in `x-aidash-csrf`; the backend also checks `Origin`. `POST /auth/logout` revokes the current browser session, while `/auth/logout-all` revokes all Aidash sessions for that external identity. A validated `POST /auth/backchannel-logout` revokes the matching provider sessions. API Bearer authentication remains available and takes precedence when an Authorization header is present.

The caller selects `x-aidash-context: mapping:<mapping-id>` or `operator` for each browser API request. The server rechecks the identity, exact mapping or operator grant, and tenant policy. The UI stores only the non-secret selector per tab; the session credential stays in an HttpOnly cookie. Operators can list `/api/dashboard/registrations`, `/api/dashboard/identities`, `/api/dashboard/mappings` and `/api/dashboard/operator-grants`; approve a pending request with `POST /api/dashboard/registrations/{id}/approve` and `{"tenant":"acme","subject":"alice"}`, reject one at `/reject`, disable a mapping at `/api/dashboard/mappings/{id}/disable`, and set an identity's operator grant at `/api/dashboard/identities/{id}/operator-grant` with `{"enabled":true|false,"expected_revision":0}` for creation or the current revision for an update. The grant list and update response include the revision; a stale update returns 409. A pending registration expires after seven days and can be resubmitted. If the identity's last mapping is disabled, it can request approval again; approval can restore the same subject with a new nonexportable credential. The first operator grant uses the existing Bearer recovery credential. A Keycloak-disabled identity requires a successful service-account status check at `/api/dashboard/identities/{id}/restore` before a fresh sign-in is permitted.

OIDC-origin execution stores an immutable run origin and rechecks its underlying nonexportable credential, mapping, external identity validity and subject policy at execution boundaries. Logging out of Aidash does not cancel admitted work. An outage beyond the 15-minute validity deadline pauses work; a fresh successful status check can resume only work waiting for that outage and only while all local authority remains valid. A definite Keycloak disablement revokes browser sessions and requires an operator identity restore followed by an authorized manual run resumption. Existing Bearer-origin work retains its credential semantics.
...
The dashboard uses a Keycloak OIDC session and a selected mapping or operator context. `/api/state` returns the authenticated access profile; the dashboard uses it to show tenant/subject identity and avoid administrator-only mesh, Marketplace and registration flows. Authorization remains enforced on the server. Invalid, expired or revoked sessions clear the dashboard's authenticated cache and return to the sign-in screen. Access-denied and reconnect messages support en-US and ja-JP.
...
`POST /api/runs/{id}/message` accepts instructions for an active run. Accepted messages are included in subsequent inference requests only after the executor can read their authorized workspace records; a later `message.read` denial pauses scoped execution. The aggregate size of run messages is bounded against the selected model's request headroom (at most one quarter, capped at 16,384 estimated serialized bytes); an oversized new message returns HTTP 400 without being recorded. Oversized messages imported from old runs remain available by message ID; the worker requires the agent to read their complete records with `workspace_read` before accepting a final answer. Read coverage is durable across context compaction. Remote messages are admitted on the executor before their home-workspace copy is delivered, and a worker retries any pending copy before inference or terminal reconciliation. For a remote run, the home node reserves the task under a row lock before executor admission; task termination returns HTTP 409 until a successful inference has observed the accepted input. A rejected admission releases its reservation. Explicit cancellation or failure supersedes an unobserved correction after its home copy is delivered, and consumes its fence atomically with task termination. Reservations for corrections not yet admitted by the executor remain active and block that transition. Terminal runs retain pending copies in a durable outbox for later delivery. Peers using an earlier protocol 0.1 implementation can still serve ordinary runs while the history and delivery extensions are unavailable; their recent keyed history is recovered from the workspace snapshot. New remote message admission requires the home peer's terminal-safe delivery capability; an older peer cannot accept a new correction under this guarantee. During rolling upgrades, a database gate rejects old local replicas' run-directed writes that lack ledger admission, including unkeyed writes while a workspace has active runs. New workspace-message handlers generate keys when callers omit them. Recovered remote runs reconcile home history before processing a pending final response. When a message arrives during final inference or while its answer is pending publication, the worker discards that answer and starts a fresh inference before publishing its final response and artifact. New messages sent once final completion starts, after cancellation or another terminal transition has been queued, or after the run is terminal, return HTTP 409 instead of a successful `sent` response. Clients should create an explicit follow-up task for a correction rejected at this boundary. Retrying a previously accepted message with the same idempotency key remains safe, including keys accepted before the run-input migration.
...
The grant destination can POST `{"grant_id":"<uuid>"}` to `/federation/v0.1/scoped/execution/grants/snapshot`. The source retains its live credential/policy lease while checking both nodes and filtering the grant's workspace snapshot. The caller cannot select a different workspace. Before returning data, the source durably records the visible task, artifact, message and event-resource dependencies under the grant, separately from local run IDs. Subsequent snapshot, description and verification requests recheck those dependencies, including originating runs' transitive registry, semantic and remote reads. Revocation or deletion of an observed resource fails closed rather than silently removing it from a context already delivered to the receiver. This source-read endpoint does not yet activate a receiver Worker or authorize home mutations.

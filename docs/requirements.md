...
| FR-HARNESS-002 providers       | `provider.rs`, shared trait and OpenAI/Anthropic adapters                                     | Adapter unit tests and HTTP protocol fixtures           |
...
| FR-HARNESS-005 context         | `context.rs`, pinned state, paired history, summary and selected events                       | Retention and budget unit tests                         |
...
Run `scripts/check.sh` for the full repeatable check, and `trunk check --all --no-fix` for lint. Generated evidence is in `.ignore/acceptance/report.json`; the dashboard screenshots below use local protocol fixtures.

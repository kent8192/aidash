...
Erasure follows tracked content dependencies into managed summaries, memories, artifacts, and execution context rather than removing only the original source. Record source identities and revisions used to generate content so the affected dependency chain can be identified. Present the impact scope for the authorized erasure operation; this is not unconditional destruction of every artifact that ever referenced the source. Make affected content unavailable for retrieval and reuse while it is erased, stripped of the affected material, or regenerated from remaining permitted sources. Search, historical views, and agent context must not reintroduce affected content while handling is pending.

Erasure of managed content does not promise recall of copies already received by people, external nodes, or services outside the operation's control. Do not claim that untracked derivatives or external copies have been erased. Retention-preserving admission control does not remove the separate ability to perform authorized content erasure.
...
Blocked work may resume without a mandatory additional human action once its blocking conditions have been resolved and its execution prerequisites are revalidated. The system verifies mechanically checkable conditions; a human or agent with resumption authority records the evidence for resolutions requiring judgment. Before admitting resumed execution, recheck the current goal revision, every remaining block, current operation authority, monetary budget, and safe Registry capacity. Resolution of one cause does not resolve another. Explicit human pause, pending approval, and cancellation remain distinct controls and are never overridden merely because new information arrives or a blocking condition clears.
...
While its delegation and read authority remain valid, the executor can read the channel's authorized retained history, including messages from before delegation, and subsequent stream updates to obtain context as needed. This includes temporary pause or block, allowing the delegate to obtain information relevant to resolving the block. Retrieve relevant portions through search or paged reads rather than inserting the entire history into every inference request. This task-bound observation permission does not grant unrelated execution, channel administration, access to owner-private references, access to other channels, or automatic participation in deciding responses to every subsequent channel message. Reads remain subject to current authorization and retained coverage.

Read access does not authorize execution resumption or paid inference that lacks budget or operation authority. Completion, cancellation, failure for which retries have been terminated, delegation withdrawal, or permission revocation ends access under that delegation. A retry must revalidate authority rather than automatically revive an expired or revoked grant; any continuing channel access requires separate authority.

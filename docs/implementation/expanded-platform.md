...
- [ ] Complete the full generation acceptance matrix, including process-level provisioning restart, all limits and denied tools. Backend PostgreSQL tests cover missing-agent completion, deduplication, concurrent quota, denied approval, nested authority/depth, credential revocation, token exhaustion, expiry and stop during inference.

...
- [x] Add individual task/artifact/message read policies to API aggregates, events, SSE frames and worker snapshots. Resolve creator/scope attributes from stored records and retain journal source-read requirements across revocation and upgrade.
...
- [x] Add bilingual generation policy controls, task assignment, request-time permission/limit review, approval/denial/stop/archive, usage and history views. Verify desktop and mobile layouts.
- [x] Complete the generation acceptance matrix: missing-agent completion, request deduplication, process restart without duplicate definitions/runs, independent count/concurrency/depth/token/call limits, denied approval, inherited permissions and rejected tools, expiry, revocation, stop during inference and retained lifecycle history. See the evidence table in [generation.md](../generation.md#acceptance-evidence).
- [x] Add an explicit approved and budgeted compaction provider contract for generated definitions, with pinned provider versions, ancestor-intersected call allowances, bounded transport and bilingual controls.
...
- Task 4, generation dashboard: the Japanese/English interface creates and edits policies, assigns a missing-agent task, reviews the pinned specification and usage, approves completion, rejects and stops later requests, archives history and toggles generation. A browser fixture proves that a policy edit after the request does not change the permission snapshot presented for approval. Desktop and mobile screenshots were inspected.
- Thirteen generation PostgreSQL tests and the API contract test passed. A regression initially returned HTTP 400 when disabling a policy after model approval revocation; disabling the unchanged definition now succeeds while re-enabling remains rejected. Pinned-specification and usage APIs retain tenant/read authorization. OpenAPI and clients expose 45 operations; Clippy, frontend compilation and Trunk pass.
- The two-node dashboard golden path passed six browser cases with four tasks/artifacts, three effects, original-run SIGKILL recovery, 169 queued events during NATS outage and 220 observed SSE events. The generation browser case was repeated after adding immutable permission review and responsive layout refinements. Evidence: `.ignore/platform/generation-ui-*` and `.ignore/dashboard-generation-*.png`. Approved compaction, remaining failure/limit cases and Tasks 2–8 release gates remain open.

- Task 4, approved compaction and final generation matrix: immutable `compactor` entries select System One explicitly, with tenant approval, generated-ancestor agreement, per-agent/total call reservations, durable attempt records, request/response/question bounds and redirect rejection. Calls never fall back to the node environment for a generated chain. Bilingual policy and approval views expose the pinned provider and call limits/usage.
- Six compaction PostgreSQL tests passed, including nested allowance/permission intersection and real worker SIGKILL during inference and compaction. Restarting retains uncertain charges and completes the original run with exactly one generated definition. Fifteen generation PostgreSQL tests passed, including independent count/concurrency/total-token rejection and a generated attribute-based tool denial that preserves its pending call and executes it once after authorized resume.
- The full Rust suite passed 71 cases before adding four final acceptance cases; those four also passed in the focused generation/compaction runs. The provider transport unit test covers request/question/response limits and redirect rejection. Clippy, generated clients, frontend production compilation and Trunk passed. The dashboard golden path passed all six browser cases, four tasks/artifacts and three effects, with original-run recovery, 175 queued outage events and 226 SSE events. Desktop/mobile generation screenshots were inspected. Evidence: `.ignore/platform/compaction-*`, `.ignore/platform/generation-matrix.log` and `.ignore/acceptance/report.json`.
- Task 4's standalone acceptance gate is complete. The remaining authorization/mesh, distributed transaction, semantic memory, A2A, cluster orchestration and integrated release gates are still open; this does not establish completion of all six requested capabilities.

- Task 2, individual resource reads: the initial regressions exposed an artifact in the aggregate state and allowed an unreadable task to be claimed. Seven PostgreSQL cases now verify per-task/artifact/message filtering, stored-author ABAC, rejection of unreadable dependencies, rollback with retained denial audit, source revocation before provider I/O and preservation of hidden records in retained journals. Newly created outputs record their own read requirements atomically, and denying either an artifact or message hides its raw producer journal.
- The upgrade test runs migration 0010 against populated older journals and verifies conservative source retention and termination of cyclic run dependencies. The generation reuse regression applies a stored task-creator condition to both executor selection and admission. Evidence is retained under `.ignore/platform/resource-*`; the remaining Task 2 and integrated release gates stay open.
- All 82 Rust tests passed on the final implementation, including the real process-restart case. Clippy with warnings denied, unchanged regenerated API/client contracts, the frontend production build and Trunk on 191 files passed. The two-node golden path passed six browser cases, four tasks/artifacts and three effects, recovered the original run after SIGKILL, retained 175 events during the NATS outage and observed 212 SSE events. This verifies the existing deployment path; the expanded Kubernetes/k3s, distributed transaction, semantic memory and A2A release gates remain pending.

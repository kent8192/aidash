...
async fn cleanup(store: &Store) -> Result<()> {
	...
	let points: Vec<Uuid> = sqlx::query_scalar(
		&sea_orm::sea_query::Query::select()
			...
			.and_where(sea_orm::sea_query::Expr::cust(
				"p.retired AND p.cleaned_at IS NULL AND NOT c.retired AND p.next_attempt <= CLOCK_TIMESTAMP()",
			))
			.order_by_expr(
				sea_orm::sea_query::SimpleExpr::from(sea_orm::sea_query::Expr::col((
					sea_orm::sea_query::Alias::new("p"),
	...
	for id in points {
		...
		let row: Option<(String, Value)> = sqlx::query_as(
			&sea_orm::sea_query::Query::select()
				...
				.and_where(sea_orm::sea_query::Expr::cust(
					"p.id = $1 AND p.retired AND p.cleaned_at IS NULL AND p.next_attempt <= CLOCK_TIMESTAMP()",
				))
		...
		if let Some((collection, config)) = row {
			...
			sqlx::query(
				&sea_orm::sea_query::Query::update()
					...
					.value(
						sea_orm::sea_query::Alias::new("cleaned_at"),
						sea_orm::sea_query::Expr::cust(
							"CASE WHEN CAST($2 AS TEXT) IS NULL THEN CLOCK_TIMESTAMP() ELSE cleaned_at END",
						),
					)
					.value(
						sea_orm::sea_query::Alias::new("next_attempt"),
	...
	for collection in collections {
		...
		let config: Option<Value> = sqlx::query_scalar(
			&sea_orm::sea_query::Query::select()
				...
				.from(sea_orm::sea_query::Alias::new("semantic_collections"))
				.and_where(sea_orm::sea_query::Expr::cust(
					"collection = $1 AND retired AND cleaned_at IS NULL AND next_attempt <= CLOCK_TIMESTAMP()",
				))
				.lock_with_behavior(
					sea_orm::sea_query::LockType::Update,
					sea_orm::sea_query::LockBehavior::SkipLocked,
		...
		if let Some(config) = config {
			...
			sqlx::query(
				&sea_orm::sea_query::Query::update()
					...
					.and_where(sea_orm::sea_query::Expr::cust("collection = $1"))
					.to_string(sea_orm::sea_query::PostgresQueryBuilder),
			)
			.bind(collection)
			.bind(failure)

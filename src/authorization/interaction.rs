...
impl Access {
    ...
    pub(crate) async fn human_reads(&mut self, workspace: Uuid, run: Uuid) -> Result<bool> {
        ...
        let requests: Vec<HumanRequest> =
            sqlx::query_as("SELECT * FROM human_requests WHERE run_id=$1 AND workspace_id=$2")
                .bind(run)
                .bind(workspace)
...
pub async fn conversation(
    ...
    let result=async {
        ...
        if entry.kind!=target_kind || !matches!(target_kind,"agent"|"cluster") {
            return Err(Error::Invalid("conversation target must be an agent or cluster".into()));
        }
        let agent=if target_kind=="cluster" {
            catalog::entry(&mut access,target,"cluster.execute").await?;
            serde_json::from_value::<EntityRef>(entry.config["coordinator"].clone())
                .map_err(|_|Error::Invalid("cluster requires an explicit coordinator agent reference".into()))?
        }else { target.clone() };
        let conversation:Conversation=sqlx::query_as("INSERT INTO conversations(id,workspace_id,target,target_kind,created_by) VALUES($1,$2,$3,$4,$5) RETURNING *")
            .bind(Uuid::new_v4()).bind(workspace.id).bind(format!("{}@{}",target.id,target.version)).bind(target_kind).bind(&identity.subject).fetch_one(&mut *access.tx).await?;
        let conversation_resource=access.conversation_resource(&conversation).await?;
        access.require(&conversation_resource,"conversation.create").await?;
        access.require(&conversation_resource,"conversation.read").await?;
        f.store.event(&mut access.tx,Some(workspace.id),"conversation.created",json!(conversation)).await?;
        access.require(&resource,"message.create").await?;
        f.store.message_in(&mut access.tx,workspace.id,&identity.subject,goal,None).await?;
...
pub async fn answer(
    ...
    let mut access = Access::begin(&f.store, identity).await?;
    let result = async {
        let request: HumanRequest = sqlx::query_as("SELECT * FROM human_requests WHERE id=$1")
            .bind(id)

...
impl Fixture {
    async fn new(allowance: Option<i64>, usage: Option<u64>) -> Self {
        ...
        if allowance.is_some() {
            ...
            assert_eq!(request(&app, &f.config.api_token, "POST", "/api/authorization/acme/catalog", json!({"entry":{"id":"embedding","version":"1.0.0"},"expected_revision":0,"enabled":true})).await.0, 200);
        }
...
async fn generated_embedding_catalog_policy_and_provider_changes_cannot_increase_authority() {
    for reason in ["catalog", "policy", "index"] {
        ...
        if reason == "catalog" {
            assert_eq!(request(&fixture.app,&fixture.f.config.api_token,"POST","/api/authorization/acme/catalog",json!({"entry":{"id":"embedding","version":"1.0.0"},"expected_revision":1,"enabled":false})).await.0,200);

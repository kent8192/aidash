...
async fn worker_remote_discovery_dependencies_survive_restart_and_hide_revoked_journals() {
    ...
    assert_eq!(request(&b_app,&b.config.api_token,"POST","/api/authorization/acme/catalog",json!({"entry":{"id":"remote-only","version":"1.0.0"},"expected_revision":2,"enabled":true})).await.0,200);
    assert_eq!(
        request(
            &a_app,
            &token,

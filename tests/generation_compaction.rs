...
async fn compaction_denial_and_catalog_revocation_prevent_disclosure() {
    for revoke_catalog in [false, true] {
        ...
        let run = long_history(&f, &assignment["generation"]).await;
        if revoke_catalog {
            assert_eq!(request(&app, &f.config.api_token, "POST", "/api/authorization/acme/catalog", json!({"entry":{"id":"jev","version":"1.0.0"},"expected_revision":1,"enabled":false})).await.0, 200);
        } else {

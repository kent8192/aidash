...
The [authorization API](docs/authorization.md) can issue revocable subject tokens for tenant-scoped workspace operations and event streams. The dashboard administration and existing agent execution workflow still use the operator token; scoped execution and authorization controls in the dashboard remain under implementation.

...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        let entries = Query::select()
            ...
            .to_owned();
        let sources = Query::select().distinct().column((Alias::new("entries"),Alias::new("id")))
            .expr(Expr::cust("entry->>'id'")).expr(Expr::cust("entry->>'version'"))
            .from_subquery(entries,Alias::new("entries"))
            .join_as(JoinType::InnerJoin,Alias::new("authorization_execution"),Alias::new("e"),Expr::col((Alias::new("entries"),Alias::new("id"))).equals((Alias::new("e"),Alias::new("run_id"))))
            .cond_where(Expr::cust("jsonb_typeof(entry)='object' AND entry ? 'id' AND entry ? 'version' AND entry ? 'kind'"))
            .to_owned();
        manager
            .exec_stmt(
                Query::insert()

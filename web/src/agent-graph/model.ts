...
export type GraphInput = {
  ...
  entries: readonly Pick<
    Entry,
    "id" | "version" | "kind" | "name" | "config"
  >[];
  runs: readonly Pick<
    Run,
    "id" | "agent_id" | "agent_version" | "task_id" | "phase" | "control"
  >[];
...
export function buildAgentGraph(
  ...
  const entries = new Map(
    input.entries
      .filter((entry) =>
        graphKinds.slice(0, 5).includes(entry.kind as EntityKind),
      )
  ...
  const addEntity = (kind: EntityKind, ref: EntityReference): string => {
    ...
    if (!nodes.has(id)) {
      ...
      nodes.set(id, {
        id,
        kind,
        entity: { id: ref.id, version: ref.version },
        name: entry ? localized(entry.name) : {},
  ...
  if (options.runtime !== false) {
    ...
    for (const run of input.runs) {
      const agentId = entityKey(input.nodeId, "agent", {
        id: run.agent_id,
      ...
      nodes.set(id, {
        ...
        name: {},
        resourceId: run.id,
        status: run.control === "PAUSED" ? "PAUSED" : run.phase,
        available: true,
      });
      nodes.set(taskId, {
        id: taskId,
        kind: "task",
        name: task ? { en: task.title } : {},
        resourceId: run.task_id,
        status: task?.status,
  ...
  const eligible = (id: string) =>
    id === rootId || kinds.has(nodes.get(id)!.kind);
  ...
  const treeEdges = new Set(
    [...visible].flatMap((id) => (parents.has(id) ? [parents.get(id)!] : [])),
  );
  const boundedEdges = [
...
export function layoutGraph(
  ...
  const occupied = new Set(
    Object.values(positions).map((p) => `${p.x},${p.y}`),
  );
  const slot = (index: number): Point => {
    ...
    return {
      x: Math.round(Math.cos(angle) * ring * 240),
      y: Math.round(Math.sin(angle) * ring * 180),
    };
  };
  let index = 0;
...
export function relationshipCurve(
  ...
  const start = {
    x: from.x + dx * sourceRadius,
    y: from.y + dy * sourceRadius,
  };

...
test("imports a GitHub Skill directory with reference files", async ({ page }) => {
  const errors = await setup(page);
  ...
  await page.route("**/api/skills/import", async (route) => {
    ...
    await route.fulfill({ json: {
      ...
      selected: request.skill_path ? {
        ...
        source: "https://github.com/example/skills/blob/abc123/skills/research/SKILL.md",
        instructions: skill,
        files: [
  ...
  await page.goto("/registry");
  await page.getByRole("button", { name: "エンティティを登録", exact: true }).click();
  const dialog = page.getByRole("dialog");
  await dialog.getByLabel("エンティティの種類").selectOption("skill");
  await dialog.getByLabel("Skillの取得元URL").fill("https://github.com/example/skills");
  await dialog.getByRole("button", { name: "URLからSkillを探す" }).click();
  await dialog.getByLabel("リポジトリ内のSkill").selectOption("skills/research/SKILL.md");
  ...
  await expect(dialog.getByText("バイナリファイルはbase64で保存されます。")).toBeVisible();
  await dialog.getByLabel("名前").fill("Research");
  await dialog.getByLabel("説明").fill("Research primary sources");
  const submitted = page.waitForRequest((request) =>
    new URL(request.url()).pathname === "/api/registry" && request.method() === "POST");
  await dialog.getByRole("button", { name: "エンティティを登録", exact: true }).click();
  expect((await submitted).postDataJSON().config).toMatchObject({
    instructions: skill,
    files: [
      { path: "references/method.md", content: "Compare primary sources." },
...
test("imports a skills.sh registry page and its supporting files", async ({ page }) => {
  const errors = await setup(page);
  const skill = "---\nname: editor\ndescription: Edit copy\n---\nRead references/style.md.\n";
  await page.route("**/api/skills/import", async (route) => {
    expect(route.request().postDataJSON().url).toBe("https://skills.sh/example/skills/editor");
    await route.fulfill({ json: {
      skills: ["SKILL.md"],
      selected: {
        path: "SKILL.md",
        ...
        instructions: skill,
        files: [{ path: "references/style.md", content: "Use clear language." }],
      },
    } });
  });
  await page.goto("/registry");
  ...
  const dialog = page.getByRole("dialog");
  await dialog.getByLabel("エンティティの種類").selectOption("skill");
  await dialog.getByLabel("Skillの取得元URL").fill("https://skills.sh/example/skills/editor");
  await dialog.getByRole("button", { name: "URLからSkillを探す" }).click();
  await expect(dialog.getByLabel("指示", { exact: true })).toHaveValue(skill);

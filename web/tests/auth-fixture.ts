...
export async function installBearerDashboard(
  ...
  await page.addInitScript(({ sessionId, context }) => {
  ...
  await page.route("**/auth/config", (route) => route.fulfill({
    status: 200, contentType: "application/json", body: JSON.stringify({ enabled: true, login_url: "/auth/login" }),
  }));
  await page.route("**/auth/session", (route) => route.fulfill({
    status: 200, contentType: "application/json",
    body: JSON.stringify({ id: sessionId, operator: !subject, mappings: subject ? [{ id: "fixture-mapping", tenant: subject.tenant, subject: subject.name }] : [] }),
  }));
  await page.route("**/auth/registration", (route) => route.fulfill({ status: 200, contentType: "application/json", body: "null" }));
  ...
  await page.route("**/api/**", (route) => route.continue({
    headers: { ...route.request().headers(), authorization: `Bearer ${currentToken}` },
  }));
  return { setToken(value: string) { currentToken = value; } };
}

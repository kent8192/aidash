...
import {
  ...
  layoutGraph,
  clampZoom,
} from "../src/agent-graph/model.ts";

const ref = (id, version = "1.0.0") => ({ id, version });
const entry = (kind, id, config = {}, version = "1.0.0") => ({
  kind,
  id,
  version,
  name: { en: id },
...
const agent = entry("agent", "agent", {
  ...
  skills: [ref("research")],
  cluster: ref("team"),
  instructions: "Mentioning other does not create an edge",
  knowledge_digest: "PRIVATE_DIGEST",
...
const base = {
  nodeId: "aidash://local",
  root,
  entries: [agent, model, tool, skill, cluster, other],
  runs: [],
  tasks: [],
};
const key = (e) => entityKey(base.nodeId, e.kind, e);
const graph = (patch = {}, options = {}) =>
  buildAgentGraph({ ...base, ...patch }, options);
const byId = (g, id) => g.nodes.find((n) => n.entity?.id === id);
const run = (id = "run-1", patch = {}) => ({
  id,
  agent_id: "agent",
  agent_version: "1.0.0",
  task_id: "task-1",
...
test("uses exact versioned references and typed configuration links", () => {
  const g = graph({
    entries: [...base.entries, entry("model", "model", {}, "2.0.0")],
  });
  assert.equal(g.rootId, key(agent));
  assert.equal(g.nodes.length, 5);
  assert.deepEqual(g.edges.map((e) => e.relation).sort(), [
    "cluster",
    "model",
    "skill",
    "tool",
...
test("returns no stale graph when the root is outside the authorized snapshot", () => {
  ...
  assert.equal(
    graph({ entries: [entry("agent", "agent", {}, "2.0.0")] }),
    null,
  );
});
...
test("ignores malformed reference fields rather than guessing IDs from text", () => {
  const malformed = entry("agent", "agent", {
    model: "model@1.0.0",
    tools: [null, 1, {}, { id: "search" }],
    skills: "research",
    cluster: [],
  });
  ...
  assert.equal(
    graph({ entries: [entry("agent", "agent", null)] }).nodes.length,
    1,
  );
});

test("deduplicates versioned references without collapsing distinct relationship types", () => {
  const duplicate = entry("agent", "agent", {
    tools: [ref("search"), ref("search")],
  });
...
test("expands explicit incoming and outgoing neighbors without text inference", () => {
  ...
  assert.ok(
    g.edges.some((e) => e.source === key(other) && e.target === key(model)),
  );
  assert.ok(
    !g.edges.some((e) => e.source === key(agent) && e.target === key(other)),
  );
});
...
test("handles cyclic cluster/coordinator configuration", () => {
  ...
  const g = graph(
    { entries: [agent, cluster, coordinator] },
    { expandedIds: [key(cluster), key(other)] },
  );
...
test("ignores expansions that are not reachable from the root", () => {
  ...
  const g = graph(
    { entries: [...base.entries, isolated, entry("tool", "secret")] },
    { expandedIds: [key(isolated)] },
  );
...
test("joins activity through exact-version runs and their explicit task IDs", () => {
  const g = graph({
    runs: [
      run(),
      run("wrong-version", { agent_version: "2.0.0" }),
      run("unrelated", { agent_id: "other" }),
    ],
...
test("runtime toggle and type filters preserve the anchor and remove dangling paths", () => {
  const input = {
    runs: [run()],
    tasks: [{ id: "task-1", title: "Task", status: "RUNNING" }],
  };
  assert.ok(
    graph(input, { runtime: false }).nodes.every(
      (n) => n.kind !== "run" && n.kind !== "task",
    ),
  );
...
test("filters are applied before the graph budget and truncation is explicit", () => {
  ...
  const many = entry("agent", "agent", {
    model: ref("model"),
    tools: tools.map((e) => ref(e.id)),
  });
...
test("limits are clamped and configuration is not displaced by busy runtime activity", () => {
  const g = graph({
    runs: Array.from({ length: 150 }, (_, i) => run(`run-${i}`)),
  });
...
test("identity includes origin, kind and version without delimiter collisions", () => {
  ...
  assert.notEqual(
    entityKey(base.nodeId, "agent", ref("a@b", "c")),
    entityKey(base.nodeId, "agent", ref("a", "b@c")),
  );
});
...
test("layout preserves positions through additions and filters, including dragged nodes", () => {
  ...
  assert.deepEqual(
    layoutGraph(g.nodes, g.rootId),
    layoutGraph([...g.nodes].reverse(), g.rootId),
  );
});
...
test("caps dense edges while retaining a connected spanning path for every displayed node", () => {
  ...
  const agents = Array.from({ length: 30 }, (_, i) =>
    entry("agent", i === 0 ? "agent" : `peer-${i}`, {
      tools: tools.map((t) => ref(t.id)),
    }),
  );
  const g = graph(
    { entries: [...tools, ...agents] },
    { expandedIds: tools.map(key) },
  );
